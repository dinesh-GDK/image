...
COLOR get_nearest_vertex(const MBVQ mbvq, const float R, const float G, const float B) {
...
std::vector<BYTE> get_color(std::vector<std::vector<std::vector<double>>> &ip_crate, unsigned int x, unsigned int y, double threshold) {
...
std::vector<BYTE> get_mbvq_color(std::vector<std::vector<std::vector<double>>> &ip_crate, unsigned int x, unsigned int y) {
    const MBVQ mbvq = get_mbvq(ip_crate[x][y][0], ip_crate[x][y][1], ip_crate[x][y][2]);
    const COLOR vertex = get_nearest_vertex(mbvq, ip_crate[x][y][0]/255., ip_crate[x][y][1]/255., ip_crate[x][y][2]/255.);

    unsigned char R = 0, G = 0, B = 0;

    if (vertex == COLOR::RED || vertex == COLOR::MAGENTA || vertex == COLOR::YELLOW || vertex == COLOR::WHITE) {
        R = 255;
    }

    if (vertex == COLOR::GREEN || vertex == COLOR::CYAN || vertex == COLOR::YELLOW || vertex == COLOR::WHITE) {
        G = 255;
    }

    if (vertex == COLOR::BLUE || vertex == COLOR::CYAN || vertex == COLOR::MAGENTA || vertex == COLOR::WHITE) {
        B = 255;
    }
    return {R, G, B};
}
...
Image error_diffusion(Image image, DIFFUSION_KERNEL kernel_type, bool isMBVQ=false, double threshold = 127.) {
