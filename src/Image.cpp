...
Image Image::operator+(const Image &rhs) {
    assert (this->_width == rhs._width && this->_height == rhs._height && this->_channels == rhs._channels);
    Image result;
...
void Image::readJpg(const std::string &filename) {
    ...
    while (cinfo.output_scanline < cinfo.output_height) {
        rowPointer[0] = &data[(cinfo.output_scanline) * this->_width * this->_channels];
    ...
    for (int i = 0; i < this->_height; i++) {
        ...
        for (int j = 0; j < this->_width; j++) {
            ...
            for (int k = 0; k < this->_channels; k++) {
                this->_crate[i][j][k] = data[(i * this->_width + j) * this->_channels + k];
            }
        }
    }
}
...
void Image::writeJpg(const std::string &filename, int quality) {
    ...
    std::vector<unsigned char> data(this->_width * this->_height * this->_channels);
    for (int i = 0; i < this->_height; i++) {
        for (int j = 0; j < this->_width; j++) {
            for (int k = 0; k < this->_channels; k++) {
                data[(i * this->_width + j) * this->_channels + k] = this->_crate[i][j][k];
            }
        }
    }
    JSAMPROW rowPointer[1];
...
Image Image::rgb_2_gray() {
    ...
    for (int i = 0; i < this->height(); i++) {
        for (int j = 0; j < this->width(); j++) {
            grayscale.set(i, j, 0, 0.257 * this->get(i, j, 0) + 0.504 * this->get(i, j, 1) + 0.098 * this->get(i, j, 2) + 16);
        }
    }
